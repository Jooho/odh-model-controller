...
var _ = Describe("AuthPolicyMatcher", func() {
	...
	Describe("FindLLMServiceFromGatewayAuthPolicy", func() {
		It("should return false when no LLMInferenceService found", func(ctx SpecContext) {
			...
			namespacedName, found := matcher.FindLLMServiceFromGatewayAuthPolicy(ctx, authPolicy, fakeClient)
		...
		It("should handle API errors gracefully", func(ctx SpecContext) {
			restrictedClient := fake.NewClientBuilder().WithScheme(scheme).Build()
			...
			namespacedName, found := matcher.FindLLMServiceFromGatewayAuthPolicy(ctx, authPolicy, restrictedClient)
