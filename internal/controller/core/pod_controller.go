...
func (r *PodReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	err := reconcileRoleBinding(ctx, r.Client, logger, req.Namespace, pod)
	if err != nil {
		return reconcile.Result{}, err
	}
...
func reconcileRoleBinding(ctx context.Context, k8sClient client.Client, logger logr.Logger, targetNamespace string, pod *corev1.Pod) error {
	if pod.Spec.ServiceAccountName != "default" {
		roleBinding := &k8srbacv1.RoleBinding{}
		if err := k8sClient.Get(ctx, types.NamespacedName{Name: constants.RayTLSSecretReaderRoleBindingName, Namespace: targetNamespace}, roleBinding); err != nil {
		...
		err := k8sClient.Update(ctx, roleBinding)
		if err != nil {
